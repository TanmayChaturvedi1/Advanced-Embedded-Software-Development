...
int main( int argc, char *argv[] )
{
    ...
    fprintf(child.fptr, "\nChild 1 Start time = %ld", start_time);
	...
	else printf(" ERROR: Enter the file name along with ./final \n");
		return -1;
}
...
void *child_function1( void* argv )
{
	...
	if ((child.fptr = fopen( child1_file, "a" )) != NULL)
	{	
...
void thread_handler(union sigval sv)
{
	...
	trigger.it_value.tv_nsec = 10^8; //10^8 nano sec is 100msec
	trigger.it_interval.tv_nsec = 10^8;
...
{
	pthread_mutex_lock(&mutex_lock);
	filestruct_t *child2_file = (filestruct_t*)argv;
	printf("100msec elapsed\n");
	char* filename;
	pid_t linux_tid = syscall(SYS_gettid);
	pthread_t posix_tid = pthread_self();
	//filename = (char*)argv;
	child.fptr = fopen( child2_file, "a" );
	if ( child.fptr != NULL)
	{
		fputs( message2, child.fptr );
		fprintf(child.fptr, "\nLinux Thread id = %d \n", linux_tid);
		fprintf(child.fptr, "Posix Thread id = %ld \n", posix_tid);
		fclose( child.fptr );	
			//pthread_mutex_unlock(&mutex_lock);
